...
def plot_faithfulness(
    ...
    for i in range(n_models):
        # Assign color based on i//2, and hatch based on i%2
        color_for_this_model = bar_colors[i // 2]  # same color for pairs (0 & 1 => color 1, 2 & 3 => color 2)
        hatch_for_this_model = hatches[i % 2]      # alternate hatch for even/odd index
        ...
        try:
            bar_container = ax.bar(
                ...
                color=color_for_this_model,
                edgecolor=frame_color,
                hatch=hatch_for_this_model,
                label=model_names[i],
                zorder=3,
            )
        except TypeError:
